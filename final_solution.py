...
class SyncService:
    def __init__(self):
        self.data_store = [] #list to store the messages received from the devices 
    ...
    def onMessage(self, data: dict):
        ...
        if data=={} :
            return {'type':'update', 'from': 1e10000000} #to handle the failures of probe request 
                                                        #Set value of 'from' large enough such that it is always greater than the 'len(self.records)'. This will ensure that devices failures are handled 
        if data['type'] == 'record':
            #Store the data in server memory
            self.data_store.append(data)
        
        elif data['type']== 'probe':
            if data['from']>=len(self.data_store):       #This will ensure that the list index is inside the range    
                return {'type':'update', 'from': 1e100000}
            value= self.data_store[data['from']]
            uuid_data= value['data']
            device_id= value['dev_id']
            device_time= value['timestamp']
            return {'type': 'update', 'from': data['from'], 'data':[{'dev_id':device_id, 'data': uuid_data,'timestamp':device_time}]}
...
def testSyncing():
    ...
    #checking that number of data sent is equal to the number of records in the server
    # total_sent=0
    # for i in range(len(devices)):   
    #     total_sent= total_sent+ len(devices[i].sent)    
    
    # print(total_sent)
    # print("\n")
    # print(len(syn.data_store))
    # print("\n")
    # print(syn.data_store)
    # print("\n")
